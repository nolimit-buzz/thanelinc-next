...
/**
 * The /contact page body. The map artwork below is layout, not copy — it stays
 * here rather than in the CMS, which is why `contact.hero-section`'s `heroVisual`
 * field is not mapped.
 */
export function ContactPageBody({ content }: { content: ContactPageContent }) {
  const email = content.channels[0];
      ...
      <main className={styles.page}>
          ...
          <div className="container">
            <div className={`${styles.headingRow} reveal`}>
              ...
              <div className={styles.heroCopy}>
                ...
                <p className={styles.heroSummary}>{content.subhead}</p>
              </div>
            </div>
            ...
            <div className={`${styles.visual} reveal delay-1`}>
              ...
              <div className={`${styles.visualCallout} reveal delay-2`}>
                <div className={styles.calloutLabel}>{content.visual.calloutLabel}</div>
        ...
        <section className={styles.contactBand}>
          <div className={`container ${styles.contactGrid}`}>
            <div>
              <h2 className={`${styles.channelHeading} reveal`}>{content.channelsHeading}</h2>
              ...
              <div className={`${styles.utilityLinks} reveal delay-3`}>
                <Link href={content.breachNote.href} className="mandate-link-check" style={{ color: "#fff", borderColor: "var(--color-teal-accent)" }}>
                  <span>{content.breachNote.label}</span><span>→</span>
                </Link>
                <Link href={content.selfCheckCta.href} className="mandate-link-check" style={{ color: "#fff", borderColor: "var(--color-teal-accent)" }}>
                  <span>{content.selfCheckCta.label}</span><span>→</span>
                </Link>
              </div>
            </div>
