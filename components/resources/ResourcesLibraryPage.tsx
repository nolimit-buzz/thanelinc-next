    ...
    <>
      <main className={styles.page}>
        ...
        <section className={styles.library}><div className="container">
          <div className={styles.categories}>{library.categories.map((category) => <div key={category.id}><strong>{category.label}</strong><span>{category.detail}</span><small>{category.state === "live" ? "Published now" : "Coming soon"}</small></div>)}</div>
          ...
          <ResourceCards articles={library.cards} />
        </div></section>
      </main>
      <ScrollReveals />
    </>
