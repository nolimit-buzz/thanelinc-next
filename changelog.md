...
Added `/design-review/home`: a like-for-like duplicate of the current homepage
whose only changed surface is the hero carousel. It contains four generic,
non-representational sector illustrations — female-led Tertiary Institutions
and Healthcare & Public Sector, male-led Regulated Businesses and Hospitality
Industry — and preserves the live destination mapping. These generated images
are review assets only, not Thanelinc staff, client, regulator or engagement
proof.

The artwork now sits large and uncontained over a single dark forest stage with
the site-standard chamfered top-right edge; it is no longer enclosed by a
second card. The route is `noindex` and returns 404 in production. `/`, its
current `Hero`, `homeMandateSlides`, `app/v5.css`, backend boundaries and live
routes remain unchanged.

Verification: `npx tsc --noEmit`, `npm run lint`, `git diff --check`, and
`npm run build` pass (34 static/SSG routes). A temporary local production
server returned 404 for `/design-review/home` and 200 for `/`.

---

## 2026-08-24 · About proof and client record paired

Both credential cards now carry the site’s chamfered top-right edge. The final
About CTA remains in its original full-width treatment. The preceding Cleared
Proof band is now a responsive two-column composition: proof on the left and
the existing landing-page client-logo proof system on the right, headed “Proven
Track Record” and “Who trusts us with regulated work.” No client name or logo
was newly authored; the existing landing-page component is reused.

Verification: `npx tsc --noEmit`, `npm run lint`, `git diff --check`, and
`npm run build` pass (33 static/SSG routes).

---

## 2026-08-24 · About proof pathway and credentials refined

Removed the duplicate “Before you start” panel: the cleared-proof panel is now
the sole About-page route to How We Work. The two credential cards are equal
height, with the separate registration rendered on the approved teal accent
field and contrast-adjusted copy. The licence explanation and Team introduction
now use the same body scale as the About hero description.

---

## 2026-08-24 · About page flow refined

The unboxed “What the licence changes” treatment now aligns with the page
container and uses a readable editorial split rather than an inset panel. The
Team heading now follows the shared Outfit display scale and weight. The cleared
proof and “Before you start” content now share one contained two-part band,
removing the oversized duplicate dark panels while retaining both pathways.

---

## 2026-08-24 · Mandate cards linked to live sector routes

The homepage’s Hospitality Industry card now leads to Regulated Businesses;
Healthcare & Public Sector now leads to the Public Sector route. No unsupported
sector route was introduced.

---

## 2026-08-24 · About, Team and Credentials consolidated

`/about` now contains the About introduction, Team and Credentials as one
continuous page, with anchored `#team` and `#credentials` sections. The former
`/about/team` and `/about/credentials` routes return 307 redirects to those
anchors, preserving direct links while eliminating the duplicate page chrome.
Navigation, search and credential calls now use the anchored About URLs. The
single closing CTA is “Ready to confirm what applies to your organisation?”

No backend, Strapi, certificate-asset or `app/v5.css` work was added.

Verification: `npx tsc --noEmit`, `npm run lint`, `git diff --check`, and
`npm run build` pass (33 static/SSG routes). A fresh local HTTP check returned
200 for `/about`, found both section IDs and the supplied CTA, and confirmed
the two 307 redirects.

---

## 2026-08-24 · Team listing order set explicitly

The Team-card order is now Ogho Emore, Tsenyon Dariem, Chukwuweike Karl Ogwu,
then Emmanuella Uyaelumuo. The typed content model uses `displayOrder`, so the
sequence is preserved independently of source-array order and can be carried to
Strapi later.
...
## 2026-08-24 · Team-card descriptions removed

Removed the short descriptions from the Team-card surface. Cards now contain
only the intentionally aligned two-line name, two-line role and `Read profile`
action; the complete approved biography remains in the popup.
...
## 2026-08-24 · Team card typography aligned to a fixed two-line rhythm

The Team-card name now has deliberate first-name and surname lines; role and
summary each reserve exactly two lines. This removes ad-hoc wrapping and aligns
the text blocks and `Read profile` actions across the four-card row.

Verification: `npx tsc --noEmit`, `npm run lint`, and `git diff --check` pass.

---

## 2026-08-24 · Team cards restored to full-bleed portrait flow

Removed the reduced foreground portrait and blurred duplicate-background
treatment after owner review. Every Team card now uses one full-bleed approved
portrait, restoring Ogho Emore's original presentation and matching Tsenyon
Dariem's natural background flow. The full-profile modal remains unchanged.

Verification: `npx tsc --noEmit`, `npm run lint`, and `git diff --check` pass.

---

## 2026-08-24 · Team portraits reduced to Tsenyon reference scale

Reduced Ogho, Karl and Emmanuella's foreground portraits to Tsenyon Dariem's
smaller visual scale. Each card retains a softened, darkened version of its
own approved image as a full-bleed background, so the reduced portrait has a
deliberate framed treatment rather than exposed empty space. Profile modals
continue to show the full portrait.
...
## 2026-08-24 · Team portrait focal alignment

Aligned the Team portraits to Tsenyon Dariem's reference framing. The typed
Team image contract now carries per-person focal point and crop-scale metadata,
used consistently by both the portrait cards and the full-profile modal. This
keeps heads in the same visual band without altering the card dimensions or
replacing any approved imagery.

Verification: `npx tsc --noEmit`, `npm run lint`, and `git diff --check` pass.

---

## 2026-08-24 · Team profile modal moved to the document root

Fixed the profile-popup width defect reported in browser review. The modal had
been rendered inside the card's transformed reveal ancestor, which made its
fixed positioning resolve against the narrow card rather than the viewport. It
now mounts through a React portal at `document.body`, preserving the full
viewport overlay and the intended two-column editorial card.
...
## 2026-08-24 · Team profiles open in editorial modal cards

`Read profile` now opens a white, portrait-led editorial modal containing the
complete approved biography. It locks background scrolling while open, supports
Escape, close-control and backdrop dismissal, contains keyboard focus, and
returns focus to the triggering card on close. The Team surname and profile
accent now use Thanelinc's teal secondary colour rather than the green from the
reference image.

Verification: `npx tsc --noEmit`, `npm run lint`, `git diff --check`, and
`npm run build` pass. The fresh local route has one H1 and four `Read profile`
buttons, with no residual inline biography disclosure.

---

## 2026-08-24 · Fourth Team portrait recovered from supplied document

The owner confirmed that the Team Google document is an approved portrait
source. Its embedded Chukwuweike Karl Ogwu portrait has been extracted,
visually verified and stored locally; the fourth leadership card now renders.
No generic portrait was used and no LinkedIn placeholder was added.

Verification: `npx tsc --noEmit`, `npm run lint`, `git diff --check`, and
`npm run build` pass. A fresh local build returns HTTP 200 for `/about/team`
and all four portrait assets; it has one H1 and renders all four approved names.

---

## 2026-08-24 · Team leadership cards implemented with approved portraits

`/about/team` now uses the approved leadership header and responsive,
portrait-led card treatment. It renders the current owner-approved copy for
Ogho Emore, Emmanuella Uyaelumuo and Tsenyon Dariem with locally stored,
individually named portraits; each card has a grayscale presentation, readable
gradient treatment and a keyboard-operable full-profile disclosure. There are
no LinkedIn actions because no approved profile URLs were supplied.

At the time of this entry, Chukwuweike Karl Ogwu's profile was deliberately
excluded pending portrait confirmation. The newer entry records the approved
portrait embedded in the owner-supplied Team document. The Team component
remains a data-only seam for the Strapi implementation team; no backend or API
work was added.

Verification: `npx tsc --noEmit`, `npm run lint`, `git diff --check`, and
`npm run build` pass. The local route has one H1, returns HTTP 200 with all
three portrait assets returning HTTP 200, and contains no Karl card or
LinkedIn placeholder.

---

## 2026-08-24 · Credentials made description-only; Team profile text cleared

`/about/credentials` no longer renders certificate previews, exposes certificate
PDF URLs, or offers View/Download controls. Its metadata and on-page copy now
describe the two credentials without implying public document inspection. The
underlying static assets were deliberately retained but are no longer referenced
by the route.

The owner approved the Team-page leadership-card plan and the four supplied
profile texts. W-042 records clearance for Ogho Emore, Chukwuweike Karl Ogwu,
Emmanuella Uyaelumuo and Tsenyon Dariem. This entry records the state before
the owner subsequently supplied three named portrait assets; see the newer
Team leadership-card entry for the current implementation state.

Verification: `npx tsc --noEmit`, `npm run lint`, `git diff --check`, and
`npm run build` pass. The build generated 33 static/SSG routes; the compiled
Credentials route contains none of the retired certificate-link text or asset
paths.
